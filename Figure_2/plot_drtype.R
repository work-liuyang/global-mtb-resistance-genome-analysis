...
colors <- data.frame(
  drtype = c("Sensitive", "RR-TB", "HR-TB", "MDR-TB","Pre-XDR-TB", "XDR-TB", "Other"), 
  color = c('#7C93C6', '#009FC3', '#4D6A74', '#F4805D', '#9BD5E7', '#B1BAC1', '#EE2E22'),
  row.names=1
)


lag <- read.table('drtype_stats.xls', sep = "\t", header = T)
names(lag) <- c("Area", "drtype", "count")
lag <- arrange(lag,Area,drtype)

index <- rownames(colors)
...
for (x in 1:nrow(size)) {
  subdrtypes <- lag[lag$Area == size$area[x], ]$drtype
  subcolors <- colors[subdrtypes, "color", drop=FALSE]
  subcolors <- subcolors[index, , drop=FALSE]
  subcolors <- subcolors[!is.na(subcolors$color), , drop=FALSE]

  sub_plot <- ggplot(lag[lag$Area == size$area[x], ], aes(x = 1, y = count, fill = factor(drtype, levels=rownames(subcolors)))) +
    geom_bar(width = 0.7, stat = "identity") +
    coord_polar("y", start = 0) +
    theme_void() +
    theme(legend.position = "None") +
    xlim(0, 1.5) +
	scale_fill_manual(values=subcolors$color)
...
gg <- gg + 
  geom_bar(data = lag, 
           aes(x = 0, fill = factor(drtype, levels=index), y = 0), stat = "identity") +
  scale_fill_manual(name = "DR-type", values = colors$color)+
...
ggsave('drtype_piemap.pdf', gg, height=10, width=18)
